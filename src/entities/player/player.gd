...
enum State { IDLE, MOVING, PROJECTILE, STUCK, TONGUE_EXTEND, TONGUE_RETRACT }

# --- NODE REFERENCES ---
# Ensure these nodes exist in your Player Scene!
...
func _process_tongue_extend(delta: float) -> void:
	...
	if result:
		...
		if collider.is_in_group("shield"):
			print("Tongue blocked.")
			_change_state(State.TONGUE_RETRACT)
		elif collider.is_in_group("prey") and collider is CharacterBody2D:
			_catch_prey(collider as CharacterBody2D)
		else:
			_change_state(State.TONGUE_RETRACT)
		return
	
	if tongue_current_length >= TONGUE_MAX_LENGTH: _change_state(State.TONGUE_RETRACT)
...
func _change_state(new_state: State) -> void:
	...
	match new_state:
		...
		State.MOVING: animated_sprite.play("run")
		State.STUCK: velocity = Vector2.ZERO
